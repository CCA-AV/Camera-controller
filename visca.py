...
results = {
    ...
    "90500y0y0y0y0y0yyy0yyyyy0yyy0yff" : {"data_digits":[[4,8],[8,12],[13,14],[15,16],[16,18],[19,20],[20,22],[22,24],[26,28],[29,30]]},
    "9050000000000000000y0yyy0y0y0yff" : {"data_digits":[[19,20],[21,22],[22,24],[25,26],[27,28],[29,30]]},
    "" : {"data_digits":[[]]},
    "" : {"data_digits":[[]]},
}
...
def interpret_completion(hex_return):
    hex_return = hex_return[:3]+"y"+hex_return[4:]
    hex_return = hex_return.lower()
    if hex_return not in returns.keys():
        return None
    result = returns[hex_return]
    if result["status"] != 0:
        warn(result["text"], ViscaException)
    return result["text"]

def interpret_inquire(hex_return):
    #print(hex_return)
    result = interpret_completion(hex_return)
    if result:
        return result
    hex_return = hex_return.lower()
    for key in results:
